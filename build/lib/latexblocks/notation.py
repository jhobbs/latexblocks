...
def scan_content(content_dir: Optional[str] = None) -> Dict[str, str]:
    ...
    """
    from .config import get_config, sty_path
    content_dir = content_dir or get_config().content_dir
...
def get_registry() -> Dict[str, str]:
    ...
    if _registry is None:
        ...
        cfg = get_config()
        _registry = scan_content(cfg.content_dir) if os.path.isdir(cfg.content_dir) else {}
    return _registry
...
def refresh_registry() -> bool:
    ...
    cfg = get_config()
    old = get_registry()
    new = scan_content(cfg.content_dir) if os.path.isdir(cfg.content_dir) else {}
    if new == old:
        return False
    _registry = new
...
# --- pdflatex support: generated notation package ---

def _sty_header(name: str) -> str:
    """Package header for the generated notation .sty. The \\ProvidesPackage
    name is the output filename stem, so a consumer writing to its own
    filename gets a matching package name."""
    return (
        f"% {name}.sty — GENERATED by the site build from \\notation\n"
        ...
        f"\\ProvidesPackage{{{name}}}[2026/07/11 Generated notation macros]\n"
    )


def write_notation_sty(path: Optional[str] = None) -> bool:
    """Regenerate the pdflatex notation package. Returns True if written."""
    from .config import get_config
    path = path or get_config().notation_sty_path
    registry = get_registry()
    stem = os.path.splitext(os.path.basename(path))[0]
    lines = [_sty_header(stem)]
    for name in sorted(registry):
        lines.append(f"\\providecommand{{\\{name}}}{{{registry[name]}}}\n")
    content = "".join(lines)
    try:
